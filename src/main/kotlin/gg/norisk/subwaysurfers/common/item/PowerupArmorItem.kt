...
class PowerupArmorItem(val collectible: Collectible, settings: Settings) : ArmorItem(
    ArmorMaterials.DIAMOND,
    collectible.armorType ?: run {
        logger().error("Armor type not set for ${collectible.id}, defaulting to CHESTPLATE")
        Type.CHESTPLATE
    },
    settings,
), GeoItem {
    ...
    override fun createRenderer(consumer: Consumer<Any>) {
        consumer.accept(object : RenderProvider {
            ...
            override fun getHumanoidArmorModel(
                ...
                if (this.renderer == null) this.renderer = GeoArmorRenderer<PowerupArmorItem>(DefaultedItemGeoModel("armor/${collectible.id}".toId()))
